    ...
    private Optional<UserProduction> montaProducao(SolicitacaoProducao sp) {
        ...
        sp.entrada.producao.regras.forEach(p ->{
            ...
            try {
            	...
            	d.add(map.get("nome").toString(),  el );
			} catch (Exception e) {
        ...
        if( sp.entrada.user != null ) {
            ...
            JsonObject producao =  find(dados, "producao");//g.toJsonTree( dado).getAsJsonObject();
        	...
        	if( entradaUser.has("piquete")) {
        		int qtdPiquete = entradaUser.get("piquete").getAsInt();
        		
        		JsonObject pasto = find(dados, "pasto");
